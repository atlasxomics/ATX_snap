...
@custom_task(cpu=8, memory=264, storage_gib=2000, retries=0)
def restore_motifs_task(
    ...
    results = Path(mkdtemp(prefix="archr_motif_recovery_"))
    project = results / f"{project_name}_ArchRProject"
    # Work exclusively on a copy: ArchR writes MotifMatrix into the Arrow files.
    shutil.copytree(source, project)
    subprocess.run([
        "Rscript", "/root/wf/R/restore_motifs.R", str(project), genome.value,
        "8",
    ], check=True)
