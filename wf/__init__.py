...
def snap_workflow(
    ...
    * matrixplot_genes.png: Heatmap of top 5 genes per group (cluster, sample, or condition), ranked by the scanpy 'scores' value from `scanpy.tl.rank_genes_groups`; see [`scanpy.pl.rank_genes_groups_matrixplot`](https://scanpy.readthedocs.io/en/latest/api/generated/scanpy.pl.rank_genes_groups_matrixplot.html).
    * matrixplot_motifs.png: Heatmap of top 5 motifs per group (cluster, sample, or condition), ranked by the scanpy 'scores' value from `scanpy.tl.rank_genes_groups`; see [`scanpy.pl.rank_genes_groups_matrixplot`](https://scanpy.readthedocs.io/en/latest/api/generated/scanpy.pl.rank_genes_groups_matrixplot.html).
    * neighborhood_enrichemnt.png: See [`squidpy.pl.nhood_enrichment`](https://squidpy.readthedocs.io/en/stable/api/squidpy.pl.nhood_enrichment.html).
    * ripleys_L.png: See [`squidpy.pl.ripley`](https://squidpy.readthedocs.io/en/stable/api/squidpy.pl.ripley.html).
    * spatial_dim_page_XX.png: Spatial scatter plots for each Run colored by cluster (paginated, up to four runs per image); see [`squidpy.pl.spatial_scatter`](https://squidpy.readthedocs.io/en/stable/api/squidpy.pl.spatial_scatter.html#squidpy.pl.spatial_scatter).
    * spatial_qc_page_XX.png: Spatial scatter plots for each Run colored by number of fragments, log10(number of fragments), and TSS Enrichment score (paginated); see [`squidpy.pl.spatial_scatter`](https://squidpy.readthedocs.io/en/stable/api/squidpy.pl.spatial_scatter.html#squidpy.pl.spatial_scatter).
    * umap.png: UMAP embedding from SnapATAC2 colored by cluster; see [`snapatac2.pl.umap`](https://kzhang.org/SnapATAC2/api/_autosummary/snapatac2.pl.umap.html).
